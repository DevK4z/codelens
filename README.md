...
khi?n b?n build kh�ng c� m�y ch? d? g?i code; nh?n th? l?i kh�ng t? t?o backend.

1. Tri?n khai backend tr�n m�y c� Node.js v� Docker. C?u h�nh m�i tru?ng theo
   "backend/.env.example"; d�y l� m?u, kh�ng du?c t? d?ng n?p. Gi? production d�ng
   DockerRunner, kh�ng d?i sang LocalRunner d? vu?t l?i sandbox.
2. Build image tr�n ch�nh m�y ch?y backend: "docker build -t codelens-sandbox ./runner".
   Ch?y "npm ci && npm run build && npm start" trong "backend" v?i bi?n m�i tru?ng d� d?t.
   C?u h�nh HTTPS/reverse proxy chuy?n "/api/*" t?i c?ng backend.
3. "CORS_ORIGINS=https://devk4z.github.io" (kh�ng th�m "/codelens/").
   "HOST=0.0.0.0" cho backend c?n nh?n k?t n?i ngo�i container; n?u reverse proxy c�ng m�y,
   c� th? gi? "127.0.0.1". Kh�ng d?i HOST local ch? d? s?a frontend.
4. GitHub -> Settings -> Secrets and variables -> Actions -> Variables:
   d?t "VITE_API_BASE_URL" b?ng URL HTTPS **th?t** c?a backend, k?t th�c b?ng "/api".
   URL v� d? trong t�i li?u kh�ng ph?i m�y ch? d� tri?n khai.
5. Ch?y l?i workflow "Deploy to GitHub Pages". Bi?n Vite du?c nh�ng khi build,
   ch? s?a bi?n r?i reload trang l� chua d?.

Workflow m?i d?ng tru?c tri?n khai n?u thi?u URL, health l?i ho?c CORS kh�ng cho ph�p
origin c?a Pages v� POST JSON. "/api/health" tr? 503 khi thi?u Docker/image ? ch? d?
production. Health ch? ki?m tra h? t?ng; c?n ch?y m?t b�i m?u d? ki?m ch?ng bi�n d?ch
v� trace sau tri?n khai. Giao di?n hi?n th? l� do ki?m tra th?t b?i v� x�a th�ng b�o
k?t n?i khi ki?m tra l?i th�nh c�ng.

Ki?m tra b? x�c th?c c?u h�nh: "node --test scripts/check-backend.test.mjs".
